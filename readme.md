...
Nginx对图片等已经被压缩的资源的Gzip传输效果并不好，可以考虑使用第三方CDN(content delivery network)解决带宽问题

## 11. 流量防刷与反爬虫
- Redis提供了TTL有效期特性：超出一段时间，key会被redis删除
- 对于每一个用户，在redis中记录访问次数
     e.g. key:188:38:12:33 value: 39 超过时间: 60s
- 用户每一次访问，对应计数器+1， 超过上限就会停止服务
- 如果计数器超过100，则认为是爬虫攻击，永久加入黑名单
- 1min后key被销毁，重新开始计数
